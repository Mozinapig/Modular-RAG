...
|---------|---------|------|---------|------|
| A1 | 初始化目录树与最小可运行入口 | [x] | 2026-05-18 | ✅ 完成：目录结构、Settings 加载、验证脚本 |
| A2 | 引入 pytest 并建立测试目录约定 | [x] | 2026-05-18 | ✅ 完成：pytest 配置、8 个 smoke 测试全通过、conftest fixtures |
| A3 | 配置加载与校验（Settings） | [x] | 2026-05-18 | ✅ 完成：已在 A1 中实现 |
...
| B9 | OpenAI格式 Vision LLM 实现 | [x] | 2026-05-27 | ✅ 完成：OpenAIVisionLLM实现、29个单元测试全通过、工厂集成 |

#### 阶段 C：Ingestion Pipeline MVP

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| C1 | 定义核心数据类型/契约（Document/Chunk/ChunkRecord） | [x] | 2026-05-28 | ✅ 完成：Document/Chunk/ChunkRecord/ImageRef/RetrievalResult 类型、33个单元测试全通过 |
| C2 | 文件完整性检查（SHA256） | [x] | 2026-05-28 | ✅ 完成：FileIntegrityChecker 抽象接口、SQLiteIntegrityChecker 实现、16 个单元测试全通过、WAL 模式支持并发 |
...
| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| D1 | QueryProcessor（关键词提取 + filters） | [x] | 2026-06-09 | ✅ 完成：关键词提取、停用词过滤、filters 解析，16 个单元测试全通过 |
| D2 | DenseRetriever（调用 VectorStore.query） | [x] | 2026-06-09 | ✅ 完成：向量查询、embedding 生成、真实分数传播，13 个单元测试全通过 |
| D3 | SparseRetriever（BM25 查询） | [x] | 2026-06-09 | ✅ 完成：BM25 稀疏检索、get_by_ids 前置任务、关键词聚合、9 个单元测试全通过 |
| D4 | RRF Fusion | [x] | 2026-06-09 | ✅ 完成：RRF 融合 dense/sparse 排名、configurable k 参数、13 个单元测试全通过 |
| D5 | HybridSearch 编排 | [x] | 2026-06-09 | ✅ 完成：dense/sparse 并行检索、RRF 融合、元数据过滤、12 个集成测试全通过 |
| D6 | Reranker（Core 层编排 + Fallback） | [x] | 2026-06-09 | ✅ 完成：Core层Reranker实现、失败回退机制、14个单元测试全通过 |
| D7 | 脚本入口 query.py（查询可用） | [x] | 2026-06-09 | ✅ 完成：CLI查询脚本、参数支持、混合检索+可选rerank、13/16 E2E测试通过 |
...
| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| G1 | Dashboard 基础架构与系统总览页 | [x] | 2026-06-17 | ✅ 完成：多页面导航、ConfigService、Overview 页面、9 个单元测试全通过 |
| G2 | DocumentManager 实现 | [x] | 2026-06-17 | ✅ 完成：list/delete/stats 方法、跨存储协调、9 个单元测试全通过、ChromaStore 新增 delete_by_metadata/get_by_metadata |
| G3 | 数据浏览器页面 | [x] | 2026-06-17 | ✅ 完成：DataService 封装、文档列表/Chunk详情/图片预览、17个单元测试全通过 |
| G4 | Ingestion 管理页面 | [x] | 2026-06-17 | ✅ 完成：文件上传、实时进度展示、文档删除、8个单元测试全通过 |
| G5 | Ingestion 追踪页面 | [x] | 2026-06-17 | ✅ 完成：TraceService 实现、追踪列表/瀑布图、19个单元测试全通过 |
| G6 | Query 追踪页面 | [x] | 2026-06-17 | ✅ 完成：查询历史列表、Dense/Sparse对比、Rerank变化、10个单元测试全通过 |
...
| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| I1 | E2E：MCP Client 侧调用模拟 | [x] | 2026-06-18 | ✅ 完成：10个E2E测试全通过、子进程启动MCP服务器、tools/list和tools/call模拟完整可用 |
| I2 | E2E：Dashboard 冒烟测试 | [x] | 2026-06-18 | ✅ 完成：10个冒烟测试全通过、6个页面均可加载、无异常 |
| I3 | 完善 README（运行说明 + MCP + Dashboard） | [x] | 2026-06-18 | ✅ 完成：共682行文档、包含快速开始、配置、MCP、Dashboard、测试、FAQ等完整指南 |
| I4 | 清理接口一致性（契约测试补齐） | [x] | 2026-06-18 | ✅ 完成：补齐delete_by_metadata、大列表、特殊字符等24个边界测试 |
| I5 | 全链路 E2E 验收 | [x] | 2026-06-18 | ✅ 完成：22个验收测试、覆盖ingest/query/evaluate/dashboard完整流程 |
